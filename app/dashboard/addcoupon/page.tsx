...
import { MdOutlineErrorOutline, MdSignalCellularNodata } from "react-icons/md";
...
export default function Page() {
  ...
  const handleCardsPageChage = (newPage: number) => {
    if (newPage > 0 && newPage <= cardsLastPage) {
      setCardsCurrentPage(newPage);
    }
  };
    ...
    const fetchCards = async (page: number) => {
      try {
        ...
        if (response.status === 200) {
          ...
          setCards(data);
          setCardsCurrentPage(pagination.current_page);
          setCardsLastPage(pagination.last_page);
        }
      } catch (error: any) {
        console.log(error);
      }
    };
    fetchCards(cardsCurrentPage);
  }, [cardsCurrentPage]);
  ...
  const handleSubmit = async (e: React.ChangeEvent<HTMLFormElement>) => {
    try {
      ...
      formdata.append("usage_limit", form.usage_limit);
      if (selectedOrg.id) formdata.append("organization_id", selectedOrg.id);
      if (selectedCard.id) formdata.append("card_type_id", selectedCard.id);
      formdata.append("category_id", selectedCategories[0]);

      const response = await instance.post("/add-copone", formdata);
    ...
    >
      <div className="min-w-2xl w-full space-y-8 bg-white border dark:border-gray-700 p-8 max-md:p-2 rounded-lg shadow-lg">
        <div>
          ...
          <p className="mt-2 text-center text-sm text-gray-600">
            {"يرجى ملء النموذج أدناه لإضافة  الكوبون"}
          </p>
        </div>
        <form className="mt-8 " onSubmit={handleSubmit}>
          ...
          <div className="rounded-md shadow-sm flex flex-col gap-3">
            ...
            <div>
              ...
              <div className="flex items-center gap-2">
                <SiRelianceindustrieslimited className="w-5 h-5 text-gray-400 max-md:hidden " />
                <input
                  id="usage_limit"
                  name="usage_limit"
                  type="number"
                  className="appearance-none rounded-none relative block w-full px-3 py-2 border border-gray-300 placeholder-gray-500 text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 focus:z-10 sm:text-sm"
                  placeholder="حد الإستخدام"
          ...
          {isSearching ? (
            ...
            <div className="w-full  grid 2xl:grid-cols-4 xl:grid-cols-3 lg:grid-cols-2  max-md:grid-cols-1 gap-8 max-md:gap-2 ">
              {orgs &&
                cards.map((card: any, index) => (
                  ...
                  >
                    ...
                    />
                    <p className="text-[17px] whitespace-nowrap max-md:text-[11px]">
                      {card.title_en && card.title_en.length > 25
                        ? card.title_en.slice(0, 25) + "..."
                        : card.title_en}
                    </p>
                    <div className="w-5 h-5 bg-gray-100 rounded-full flex items-center justify-center border shadow-sm absolute bottom-2 right-2 cursor-pointer">
                      {
          ...
          {currentData.length > 0 && !isSearching && (
            <PaginationWithoutNumbers
              ...
              onPageChange={handleCardsPageChage}
            />
          )}
          ...
          <div className="mt-4">
            ...
            <div className="mt-1 grid grid-cols-2 gap-4">
              ...
              >
                ...
                <span className="mt-2 text-sm text-gray-700">مفعل</span>
              </motion.div>
              <motion.div
                className={`p-4 rounded-lg cursor-pointer flex flex-col items-center justify-center ${
                  !active
              ...
              >
                ...
                <span className="mt-2 text-sm text-gray-700">غير مفعل</span>
              </motion.div>
            </div>
          </div>
          <div className="mt-4">
            ...
            <div className="mt-1 grid grid-cols-3 max-md:grid-cols-1  gap-4">
              <motion.div
                className={`p-4 rounded-lg cursor-pointer flex flex-col  max-md:w-full items-center justify-center ${
                  status == "active"
                    ? "bg-green-100 border-2 border-green-500"
                    : "bg-gray-100 border-2 border-gray-300"
                }`}
              ...
              >
                <FaCheck className="text-green-500 text-2xl" />
                <span className="mt-2 text-sm text-gray-700">نشر</span>
              </motion.div>
              <motion.div
                className={`p-4 rounded-lg cursor-pointer flex flex-col max-md:w-full  items-center justify-center ${
                  status == "waiting"
                ...
                whileHover={{ scale: 1.05 }}
              >
                <MdOutlineErrorOutline className="text-yellow-500 text-2xl" />
                <span className="mt-2 text-sm text-gray-700">معلق</span>
              </motion.div>
              <motion.div
                className={`p-4 rounded-lg cursor-pointer flex flex-col max-md:w-full  items-center justify-center ${
                  status == "expired"
              ...
              >
                <FaTimes className="text-red-500 text-2xl" />
                <span className="mt-2 text-sm text-gray-700">غير مسموح</span>
              </motion.div>
            </div>
          </div>
      ...
      {showSuccessPopup && (
        <SuccessPopup
          ...
          message={"تم اضافة الكوبون الى قائمة الكوبونات بنجاح ."}
        />
      )}
    </motion.div>
  );
