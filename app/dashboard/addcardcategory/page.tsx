...
export default function AddCardCategory() {
  ...
  const handleSubmit = async (e: React.ChangeEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const response = await instance.post("/add-card-type-category", formData);
  ...
  return (
    <>
      <div className="w-full">
        <div className="h-fit py-8 w-full flex items-center p-6 max-md:p-2 justify-center  dark:bg-main_dash">
          {loading ? (
            <Loading />
          ) : (
            <motion.div
