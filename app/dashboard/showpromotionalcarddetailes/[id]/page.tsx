...
export default function Page({ params }: any) {
  ...
  useEffect(() => {
    const getPromotionalCarddetailes = async (page: number) => {
      try {
        setLoading(true);
        const response = await instance.get(`/card-statics/${Id}?page=${page}`);
    ...
    if (Id) getPromotionalCarddetailes(currentPage);
  }, [currentPage]);
    ...
    <>
      <div className="w-full min-h-screen overflow-auto mx-auto p-6">
        ...
        >
          ...
          <tbody>
            {promotionalCards &&
              promotionalCards.map((item: any, index) => (
                <motion.tr
                  key={index}
                ...
                >
                  ...
                  <td className="py-4 px-6 whitespace-nowrap text-sm text-gray-600">
                    <FaPen
                      ...
                      className="size-6 cursor-pointer text-sky-400"
                    />
                  </td>
                </motion.tr>
              ))}
