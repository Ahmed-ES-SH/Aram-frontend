...
import { FaSearch, FaUserAlt, FaTimes, FaPaperPlane } from "react-icons/fa";
  ...
  const handlesearch = async (page: number) => {
    try {
      ...
      const response = await instance.post(
        `/search-by-title-for-organization/${searchQuery}?page=${page}`
      );
      ...
      setError(
        "لا يوجد مركز بهذا الاسم. يرجى التحقق من الاسم وإعادة المحاولة."
      );
    } finally {
      setLoading(false);
    }
            ...
            {datausers.length == 0 && (
              <div className="flex flex-col w-full h-full">
                ...
                <div className="w-full h-full flex flex-col items-center justify-center">
                  ...
                  <p>يمكنك البحث عن المركز عن طريق اقرب الحروف الى اسمه .</p>
                </div>
              </div>
            )}
              ...
              >
                <table className="w-full border rounded-lg overflow-auto">
                  <thead className="bg-gray-200">
                    <tr>
                      <th className="text-right p-3">الشعار</th>
                      <th className="text-right p-3">الاسم</th>
                      <th className="text-right p-3">تحديد</th>
                    </tr>
                  </thead>
                  <tbody>
                    {datausers.map((org, index) => (
                      <tr
                        key={index}
        ...
        {step === 2 && selectedUser && (
          ...
          >
            <div className="flex items-center justify-between mb-4 pb-2 border-b border-main_orange">
              <h2 className="text-xl max-md:text-sm">المركز الطبى المحدد</h2>
              <button
                onClick={() => {
                  setSelectedUser(null);
                  setStep(1);
            ...
            <div className="flex items-center gap-4 mb-6 border-b border-gray-300 pb-4">
              ...
              >
                {/* صورة المستخدم */}
                <Img
                  src={selectedUser?.icon || "/logo.png"}
                  className="w-full h-full object-cover"
                />
              </motion.div>
              <div className="text-sm">
                <div>
                  <strong>اسم المركز الطبى:</strong>
                  <p className="my-1">
                    {selectedUser?.title_en + " / " + selectedUser?.title_ar ||
                      "غير متاح"}
                  </p>
                </div>
              </div>
