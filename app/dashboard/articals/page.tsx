...
export default function Main_Blog_Section() {
  const { language }: any = UseVariables();
  const [selectedCategory, setSelectedCategory] = useState<string | number>(
    "All"
  );
  const [categories, setCategories] = useState<any[]>([]); // نوع البيانات المسترجعة من API
  const [articals, setarticals] = useState<any[]>([]); // نوع البيانات المسترجعة من API
  ...
  const fetchData = async (page: number) => {
    try {
      const response = await instance.get(`/articals?page=${page}&limit=8`);
  ...
  const filteredServices =
    selectedCategory == "All"
      ? articals
      : articals.filter((service) => service.category_id == selectedCategory);
  ...
  const handleDelete = async (id: number) => {
    try {
      ...
      const response = await instance.delete(`/artical/${id}`);
      if (response.status === 200) {
        setarticals(articals.filter((slider: any) => slider.id !== id));
      }
  ...
  return (
    <>
      <section className="w-full p-6 mt-4 mx-auto bg-slate-50 dark:bg-secend_dash py-8 md:py-12 lg:py-20">
        <h1 className="mb-8 text-4xl text-gray-900 dark:text-secend_text w-fit mx-auto pb-2 border-b-2 border-main_orange">
          المقالات
        </h1>
        <div className="flex h-screen max-lg:h-fit max-lg:py-2 max-lg:px-2 flex-col md:flex-row gap-6">
          {/* الشريط الجانبي */}
          <aside className="w-full overflow-y-auto h-full max-md:h-[300px] md:w-1/4 bg-white dark:bg-secend_dash p-4 rounded-md shadow-lg">
              ...
              {filteredServices.map((card, index) => (
                ...
                >
                  ...
                  <div className="content mt-8 px-4 flex flex-col gap-2 items-start">
                    ...
                    <p className="dark:text-gray-500 text-md font-light">
                      {language === "EN"
                        ? card.content_en.slice(0, 40) + "..."
                        : card.content_ar.slice(0, 40) + "..."}
                    </p>
                  </div>
                  <div className="w-full h-0 group-hover:h-full rounded-md duration-200 overflow-hidden absolute left-0 top-0 bg-black/50 flex items-center justify-center">
                    <div className="flex items-center  gap-2">
                      <Link
                        href={`/dashboard/articals/${card.id}`}
      ...
      {showdeletepopup && (
        <div className="w-full h-screen bg-black/50 fixed top-0 left-0 z-[99999] flex items-center justify-center">
          <div className="w-[20%] max-md:w-[80%] h-fit rounded-md shadow-md py-6 flex flex-col gap-4 items-center justify-center bg-white dark:bg-secend_dash">
