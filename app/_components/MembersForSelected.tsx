...
interface props {
  setSelectedMembersProp: any;
}

export default function MembersForSelected({ setSelectedMembersProp }: props) {
  ...
  useEffect(() => {
    if (selectedUsers.length > 0) {
      setSelectedMembersProp(selectedUsers);
    }
  }, [selectedUsers]);
  ...
  const handleDelete = async (memberId: number) => {
    try {
      ...
      if (response.status === 200) {
        ...
        setSelectedUsers(
          selectedUsers.filter((user: number) => user !== memberId)
        );
