...
export default function Hero_video_dash() {
  ...
  const updateText = async (e: React.ChangeEvent<HTMLFormElement>) => {
    e.preventDefault();

    try {
      setloading(true);

      // إنشاء FormData ديناميكيًا
      const formData = new FormData();

      // إضافة النصوص من اللغات المختلفة إلى formData
      const texts = { text1, text2, text3, text4 };
      Object.entries(texts).forEach(([key, value]) => {
        if (value.AR) formData.append(`${key.toLowerCase()}_ar`, value.AR);
        if (value.EN) formData.append(`${key.toLowerCase()}_en`, value.EN);
      });

      // إضافة الفيديو إذا كان موجودًا
      if (video) formData.append("video", video);

      // إرسال الطلب الأول لتحديث النصوص والفيديو
      const response = await instance.post(`/update-videoscreen`, formData);

      // تحديث الفيديو بناءً على الاستجابة
      if (response.data?.data?.video) {
        setvideo(response.data.data.video);
      }

      // إرسال الطلب الثاني لتحديث حالة الشاشة الرئيسية
  ...
  return (
    <>
      ...
      >
        <form
          onSubmit={updateText}
          className="w-full py-2 px-2 dark:bg-secend_dash flex items-center justify-center flex-col"
        >
        ...
        <div className="relative h-screen flex items-center justify-center w-full bg-black">
          ...
          {typeof video == "string" ? (
            <video
              ...
              src={video ? video : "/videos/about_video.mp4"}
              autoPlay
              loop
              muted
              playsInline
