...
export default function SignupForm() {
  ...
  const handleSubmit = async (e: React.ChangeEvent<HTMLFormElement>) => {
    ...
    if (PasswordStrengthProps < 2) {
      setloading(false);
      setpassworderror({
        AR: "من فضلك تأكد من كتابة كلمة سر مناسبة ",
        EN: "Please make sure to type an appropriate password.",
      });
      return null;
    }
              ...
              ${
                form.gender === option.value
                  ...
                  {errors && errors["user_gender"] && (
                    <p className="text-red-400 my-3">{errors["user_gender"]}</p>
                  )}
                </motion.div>
