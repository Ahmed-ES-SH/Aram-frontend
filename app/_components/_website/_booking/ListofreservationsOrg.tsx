...
import {
  ...
  FiHome,
  FiFileText,
} from "react-icons/fi"; // استيراد الأيقونات
...
export default function ListofreservationsUser() {
  ...
  const [unAcceptedPopup, setUnAcceptedPopup] = useState(false);
  ...
  const deleteBooking = async (booking: any) => {
    try {
      ...
      const response = await instance.post(
        `/book/${booking.id}/${null}/${booking?.organization?.id}`
      );
      if (response.status == 200) {
  ...
  const acceptedToggle = (item: any) => {
    setAcceptedPopup(true);
    setSelectedBooking(item);
  };
  const unAcceptedToggle = (item: any) => {
    setUnAcceptedPopup(true);
    setSelectedBooking(item);
  };

  const CloseAccaptedPopup = () => {
    setAcceptedPopup(false);
  };
  const CloseUnAccaptedPopup = () => {
    setUnAcceptedPopup(false);
  };
  ...
  return (
    <>
      <div className="w-full mb-2 h-screen mt-20 flex items-center">
        {/* الفلاتر */}
        <motion.div
          initial={{ x: -50, opacity: 0 }}
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.5 }}
          className="w-[20%] h-full bg-gray-200 dark:bg-gray-800 dark:text-secend_text p-4"
        >
        ...
        <motion.div
          ...
          animate={{ x: 0, opacity: 1 }}
          transition={{ duration: 0.5 }}
          className="flex-grow overflow-auto h-full min-h-screen bg-white dark:bg-main_dash rounded-md p-4"
        >
          <h2
          ...
          {filteredBookings.length > 0 ? (
            <div className="w-full rounded-md">
              <table
                className={`w-full bg-white dark:bg-secend_dash dark:text-white rounded-md shadow border ${tableBorderColors[filter]}`}
              >
                ...
                <tbody>
                  {filteredBookings.map((booking: any) => (
                    ...
                    >
                      ...
                      <td className="p-2">{booking.book_day}</td>
                      <td className="p-2">{`${booking.book_time} - ${booking.expire_in}`}</td>
                      <td
                      ...
                      <td className="p-2">
                        {booking?.user?.name || "User Deleted This booking"}
                      </td>
                      <td className="p-2">
                        {booking.status === "waiting" ? (
                          <div className="flex items-center gap-3">
                            <button
                              onClick={() => acceptedToggle(booking)}
                              className="px-4 py-2 bg-green-400 rounded-md shadow-sm hover:bg-white hover:text-black hover:border-green-400 hover:scale-105 duration-150 border border-transparent text-white text-center"
                            >
                              <MdDone />
                            </button>
                            <button
                              onClick={() => unAcceptedToggle(booking)}
      ...
      {isPopupVisible && (
        <SuccessPopup
          message={
            language == "EN" ? successPopupMessage.en : successPopupMessage.ar
          }
