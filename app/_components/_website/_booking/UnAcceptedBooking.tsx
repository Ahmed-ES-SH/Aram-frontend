...
export default function UnAcceptedBooking({
  selectedBooking,
  setLoading,
  onClose,
  setIsPopupVisible,
  ...
  const translations: any = {
    ...
    EN: {
      confirmAcceptance: "rejection Booking",
  ...
  const [notification, setNotification] = useState(
    selectedBooking.organization.unaccaptable_message
  );
  const [editNotification, setEditNotification] = useState(false);

  const handleUnAccepted = async (selectedBook: any, book_status: string) => {
    try {
      setLoading(true);
      ...
      const res = await instance.post(
        `/sendUnAccaptedNotification/${selectedBook.user.id}/${selectedBook.organization.id}`,
        formData
      );
      if (res.status == 200) {
        ...
        setIsPopupVisible(true);
        setsuccessPopupMessage({
          ar: "تم  إلغاء طلب الحجز وتم إرسال اشعار الى حساب المستخدم ",
          en: "The reservation request has been approved and a notification has been sent to the user's account",
        });
        onClose();
      }
  ...
  return (
    <>
      <div className="w-full h-screen fixed top-0 left-0 bg-black/50 flex items-center justify-center">
        ...
        >
          {/* Header */}
          <h3 className="text-xl font-semibold mb-6 pb-2 border-b border-red-500 w-fit mx-auto text-center">
            {t.confirmAcceptance}
          </h3>
          ...
          <div className="flex relative overflow-hidden group flex-col gap-2 my-2 w-full pt-2 border-t border-main_orange">
            <label className="block text-sm font-medium text-gray-700  dark:text-gray-300">
              {language == "EN"
                ? "A notification will be sent to the customer about the cancellation of his reservation request, the content of which is the text Rejection of reservation from the center’s profile. You can click on the edit mark to modify the content of the notice."
                : " سيتم إرسال إشعار الى العميل بإلغاء  طلب الحجز الخاص به محتواه نص رفض الحجز من الملف الشخصى الخاص بالمركز يمكنك الضغط على علامة التعديل لتعديل محتوى الإشعار ."}
            </label>
            <textarea
              placeholder={
          ...
          <div className="flex justify-between space-x-4">
            <motion.button
              ...
              onClick={() => handleUnAccepted(selectedBooking, "unacceptable")}
